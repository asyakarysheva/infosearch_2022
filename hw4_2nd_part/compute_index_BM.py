...
def computing_index_answers(answers_preprocessed):  # индексация корпуса
    # создание матрицы tf + понадобится для индексации запроса
    count_vectorizer = CountVectorizer()
    count = count_vectorizer.fit_transform(answers_preprocessed)
    tf = count
    # для расчета idf
    tfidf_vectorizer = TfidfVectorizer(use_idf=True, norm='l2')
    tfidf = tfidf_vectorizer.fit_transform(answers_preprocessed)
    ...
    answers_matrix = sparse.csr_matrix((values, (rows, cols)))  # создание матрицы
    return answers_matrix, count_vectorizer


def computing_index_questions(questions_preprocessed, count_vectorizer):  # индексация запроса
    questions_matrix = count_vectorizer.transform(questions_preprocessed)
