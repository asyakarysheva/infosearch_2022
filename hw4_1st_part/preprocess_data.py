...
def preprocess_text(answers, mystem):
    texts_preprocessed = []
...
def data_preprocessing(answers):  # функция, которая которая загружает Mystem и предобрабатывает тексты
    mystem = load_mystem()
    texts_preprocessed = preprocess_text(answers=answers, mystem=mystem)
