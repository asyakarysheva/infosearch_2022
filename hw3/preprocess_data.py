...
def preprocess_text(questions, mystem):
    texts_preprocessed = []
...
def data_preprocessing(questions):  # функция, которая которая загружает Mystem и предобрабатывает тексты
    mystem = load_mystem()
    texts_preprocessed = preprocess_text(questions=questions, mystem=mystem)
